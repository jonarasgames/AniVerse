...
function openEpisode(anime, seasonNumber, episodeIndex, isAutoAdvance = false) {
  try {
    ...
    // Try to restore playback position from profile's continue watching (only if not auto-advance)
    let resumeTime = 0;
    if (!isAutoAdvance && window.profileManager) {
...
// Update auto-advance calls in the ended event listener
  ...
  if (window.currentAnime && window.currentWatchingAnime) {
    ...
    if (currentSeason && currentSeason.episodes) {
      ...
      if (nextEpisodeIndex < currentSeason.episodes.length) {
        ...
        setTimeout(() => {
          window.openEpisode(window.currentAnime, window.currentWatchingAnime.season, nextEpisodeIndex, true);
        ...
        if (nextSeason && nextSeason.episodes && nextSeason.episodes.length > 0) {
          ...
          setTimeout(() => {
            window.openEpisode(window.currentAnime, nextSeason.number, 0, true);
          }, 1000);
        } else {
