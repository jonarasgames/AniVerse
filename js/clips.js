(function () {
  ...
  function stopDisplayStream() {
    if (!activeDisplayStream) return;
    activeDisplayStream.getTracks().forEach((track) => {
  ...
  async function getDisplayMediaStream() {
    ...
    // Método totalmente diferente: sempre gravar a aba/tela escolhida pelo usuário.
    // Isso evita incompatibilidades de captureStream/canvas no player.
    try {
      ...
      console.warn('Usuário cancelou/negou getDisplayMedia:', error);
      return null;
    }
  }
  ...
  async function startRecording() {
    ...
    if (!stream) {
      alert('Não foi possível iniciar a gravação de clipe. Permita capturar a aba atual quando solicitado.');
      return;
    }
