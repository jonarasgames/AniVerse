(function () {
  ...
  function stopCanvasLoop() {
    if (canvasFrameLoop) {
      cancelAnimationFrame(canvasFrameLoop);
      canvasFrameLoop = null;
    }
    activeCanvas = null;
  }
  ...
  function getCanvasStream(player) {
    try {
      ...
      const drawFrame = () => {
        try {
          ...
          canvasFrameLoop = requestAnimationFrame(drawFrame);
        } catch (error) {
          console.warn('Falha ao desenhar frame no canvas (possível CORS):', error);
          stopCanvasLoop();
        }
      };
      ...
      console.warn('Falha ao criar stream via canvas:', error);
      stopCanvasLoop();
      return null;
    }
  }
  ...
  async function acquireRecordStream(player) {
    // Estratégia nova: canvas primeiro (método alternativo), depois captureStream, depois getDisplayMedia.
    const canvasStream = getCanvasStream(player);
  ...
  async function startRecording() {
    ...
    if (!stream) {
      alert('Não foi possível iniciar a gravação de clipe. Tente Chrome/Edge atualizado e permita captura quando solicitado.');
      return;
    }
